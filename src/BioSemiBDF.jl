module BioSemiBDF
  ...
  * status Array
  ...
  function bdf2matrix(bdf, num_channels, channels, scale_factor, num_data_records, num_samples, sample_rate)
    ...
    dat    = Matrix{Float32}(undef, length(channels)-1, (num_data_records*num_samples[1]))
    trig   = Array{Int16}(undef, num_data_records*num_samples[1])
    status = Array{Int16}(undef, num_data_records*num_samples[1])
    pos = 1;
        ...
        if chan in channels  # selected channel
          if chan < num_channels
            for samp = 1:num_samples[1]
              dat[idx, rec*num_samples[1]+samp] = Float32(((Int32(bdf[pos]) << 8) | (Int32(bdf[pos+1]) << 16) | (Int32(bdf[pos+2]) << 24)) >> 8) * scale_factor[chan]
              pos += 3
            end
          else  # last channel is always Status channel
            for samp = 1:num_samples[1]
              trig[rec*num_samples[1]+samp] = ((Int16(bdf[pos])) | (Int16(bdf[pos+1]) << 8))
              status[rec*num_samples[1]+samp] = Int16(bdf[pos+2])
              pos += 3
            end
          end
          idx += 1
    ...
    time = collect(0:size(dat, 2) - 1) / sample_rate[1]

    return dat, time, trig, status
  ...
  * BioSemiData struct
  ...
  write_bdf(dat1)
  """
  function write_bdf(bdf::BioSemiData, filename::String="")

    isempty(filename) ? fid = open(bdf_in.header["filename"], "w") : fid = open(filename, "w")

    write(fid, 0xff)
    [write(fid, UInt8(i)) for i in bdf.header["id2"]]
    [write(fid, UInt8(i)) for i in bdf.header["text1"]]
    [write(fid, UInt8(i)) for i in bdf.header["text2"]]
    [write(fid, UInt8(i)) for i in bdf.header["start_date"]]
    [write(fid, UInt8(i)) for i in bdf.header["start_time"]]
    [write(fid, UInt8(i)) for i in rpad(string(bdf.header["num_bytes_header"]), 8)]
    [write(fid, UInt8(i)) for i in rpad(bdf.header["num_bytes_header"], 44)]
    [write(fid, UInt8(i)) for i in rpad(string(bdf.header["num_data_records"]), 8)]
    [write(fid, UInt8(i)) for i in rpad(string(bdf.header["duration_data_records"]), 8)]
    [write(fid, UInt8(i)) for i in rpad(string(bdf.header["num_channels"]), 4)]
    [write(fid, UInt8(j)) for i in bdf.header["channel_labels"] for j in rpad(i, 16)]
    [write(fid, UInt8(j)) for i in bdf.header["transducer_type"] for j in rpad(i, 80)]
    [write(fid, UInt8(j)) for i in bdf.header["channel_unit"] for j in rpad(i, 8)]
    [write(fid, UInt8(j)) for i in bdf.header["physical_min"] for j in rpad(i, 8)]
    [write(fid, UInt8(j)) for i in bdf.header["physical_max"] for j in rpad(i, 8)]
    [write(fid, UInt8(j)) for i in bdf.header["digital_min"] for j in rpad(i, 8)]
    [write(fid, UInt8(j)) for i in bdf.header["digital_max"] for j in rpad(i, 8)]
    [write(fid, UInt8(j)) for i in bdf.header["pre_filter"] for j in rpad(i, 80)]
    [write(fid, UInt8(j)) for i in bdf.header["num_samples"] for j in rpad(i, 8)]
    [write(fid, UInt8(j)) for i in bdf.header["reserved"] for j in rpad(i, 32)]

    dat              = round.(Int32, (bdf.data ./ bdf.header["scale_factor"][1:end-1]))
    trigs            = bdf.triggers["raw"]
    status           = bdf.status
    num_data_records = bdf.header["num_data_records"]
    num_samples      = bdf.header["num_samples"][1]
    num_channels     = bdf.header["num_channels"]

    dat = matrix2bdf(dat, trigs, status, num_data_records, num_samples, num_channels)

    write(fid, Array{UInt8}(dat))
    close(fid)
  ...
  """
  mat2bdf(dat_in, trigs, status, num_data_records, num_samples, num_channels)
  Internal functon used within write_bdf to write Julia BioSemiBDF data matrix
  to bdf 24bit file format.
  """
  function matrix2bdf(dat_in, trigs, status, num_data_records, num_samples, num_channels)

    dat_out = Array{UInt8}(undef, 3*(num_data_records*num_channels*num_samples))
    pos = 1
    for rec = 0:(num_data_records-1)
      for chan = 1:num_channels
        if chan < num_channels
          for samp = 1:num_samples
            value          = dat_in[chan, rec*num_samples + samp]
            dat_out[pos  ] = (value % UInt8)
            dat_out[pos+1] = ((value >>  8) % UInt8)
            dat_out[pos+2] = ((value >> 16) % UInt8)
            pos += 3
          end
          ...
          for samp = 1:num_samples
            ...
            dat_out[pos  ] = trig_val % UInt8
            dat_out[pos+1] = (trig_val >> 8) % UInt8
            dat_out[pos+2] = (status_val) % UInt8
            pos += 3
          end
        end
      end
  ...
  function crop_bdf(bdf_in::BioSemiData, crop_type::String, val::Array{Int}, filename::String)
    ...
    !crop_type in ["triggers", "recrods"] && error("crop_type should be one of triggers or records")
